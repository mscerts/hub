...
<script>
	class StarlightTabs extends HTMLElement {
		...
		constructor() {
			...
			// Update href to use tab label instead of panel ID
			const label = StarlightTabs.#getTabLabel(tab);
			if (label) {
				const hashLabel = label.toLowerCase().replace(/\s+/g, '-');
				tab.href = '#' + encodeURIComponent(hashLabel);
			}
			...
			if (hash) {
				const tabIndex = this.tabs.findIndex(tab => 
					StarlightTabs.#getTabLabel(tab)?.toLowerCase().replace(/\s+/g, '-') === hash.toLowerCase()
				);
				if (tabIndex > 0) {
		...
		switchTab(newTab: HTMLAnchorElement | null | undefined, index: number, shouldSync = true) {
			...
			// Update URL hash with tab label for shareable links
			const label = StarlightTabs.#getTabLabel(newTab);
			if (label && shouldSync) {
				const hashLabel = label.toLowerCase().replace(/\s+/g, '-');
				history.replaceState(null, '', '#' + encodeURIComponent(hashLabel));
			}
