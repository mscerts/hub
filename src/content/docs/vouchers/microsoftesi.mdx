---
title: Organizational Skilling (Microsoft ESI)
description: Discount opportunities for Microsoft customers or partners through the Microsoft Organizational Skilling (ESI) program.
voucherCategory: "50%"
---
import { LinkCard } from '@components/docs';

<LinkCard title="Microsoft Organizational Skilling (ESI)" href="https://esi.microsoft.com/" target="_blank" />

:::note
50% voucher for eligible exams
:::

If your company is a big Microsoft customer or partner, there is a high chance your company is enrolled into this program. You can check it by visiting the URL and typing in your work email address. 

You get access to:
- Live trainings from Microsoft Certified Trainers
- And 50% off vouchers for:
  - All Azure Exams
  - All Security, Compliance, and Identity Exams
  - All Power Platform Exams
  - All Dynamics 365 Exams **except** MB-335
