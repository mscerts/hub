...
- **`fabric-docs` currently unavailable (observed 2026-08-31/09-01):** `git clone`/`ls-remote` against `https://github.com/MicrosoftDocs/fabric-docs.git` returns `remote: Repository not found`, not a rate-limit or network error — confirmed on repeated tries. Its private `-pr` counterpart (`fabric-docs-pr`) is still live and actively updated (per a current live Fabric page's `original_content_git_url`), so the naming convention still points to `fabric-docs` as the intended public mirror; it isn't a rename. Cause unknown (could be a temporary administrative action or outage). Left in `REPOS` as-is since the existing `failedRepos` handling already does the right thing (logs it, keeps the other repos, still commits what succeeded, exits non-zero so CI opens an issue) — no code change needed unless this persists across several scheduled runs, in which case search for a renamed replacement repo the same way other gaps in this doc were resolved.
...
- **Defender for IoT: elevated content churn observed (2026-09-01):** three different `azure/defender-for-iot/*` pages turned up broken across three separate random-sample link-check runs in the same session (`microsoft-defender-iot`, `enterprise-iot-manage`, `configure-permissions`) — consistent with that product's docs being mid-migration to "Defender portal" branding. Not a script bug; flagged here in case a future pass wants to specifically re-verify all `defender-for-iot` entries rather than wait for random sampling to catch them one at a time.
...
- This does **not** replace the site's own `lychee`-based link checker (see Link Checker section above) — that one validates internal links in the *built site*; this one validates external Microsoft Learn URLs in the *research cache*, which the site doesn't consume directly.
